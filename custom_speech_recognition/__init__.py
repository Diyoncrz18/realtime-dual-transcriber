...
class Recognizer(AudioSource):
    ...
    def recognize_google(self, audio_data, key=None, language="en-US", pfilter=0, show_all=False, with_confidence=False):
        ...
        The Google Speech Recognition API key is specified by ``key``. If not specified, it uses the ``GOOGLE_SPEECH_RECOGNITION_API_KEY`` environment variable.
        ...
        if key is None:
            key = os.environ.get("GOOGLE_SPEECH_RECOGNITION_API_KEY") or None
        if key is None:
            raise RequestError("missing Google Speech Recognition API key; pass key or set GOOGLE_SPEECH_RECOGNITION_API_KEY")
